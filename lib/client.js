      ...
      const serverCard = (status) => {
        ...
        return h('div', { className: 'mcp-oauth-card', key: status.id },
          ...
          h('div', { className: 'mcp-oauth-grid' },
            ...
            h('div', null, h('div', { className: 'mcp-oauth-label' }, '工具数量'), h('div', { className: 'mcp-oauth-tools' }, String(status.toolCount ?? 0))),
          ),
          status.message && h('div', { className: 'mcp-oauth-message' }, status.message),
          h('div', { className: 'mcp-oauth-actions' },
            status.enabled !== false && h('button', { className: 'mcp-oauth-button primary', disabled: !!busy, onClick: () => operation(`reconnect:${status.id}`, 'reconnect', { serverName: status.serverName }) }, busy === `reconnect:${status.id}` ? '正在连接…' : '重新授权 / 重新连接'),
      ...
      return h('div', { className: 'mcp-oauth-page' },
        h('div', { className: 'mcp-oauth-head' },
          h('div', null,
            ...
            h('p', { className: 'mcp-oauth-subtitle' }, '统一管理 HTTP、OAuth、Bearer 和本地命令型 MCP 服务。'),
          ),
        ...
        showAdd && h('form', { className: 'mcp-oauth-form', onSubmit: addServer },
          h('div', { className: 'mcp-oauth-form-title' }, '添加 MCP 服务'),
          h('div', { className: 'mcp-oauth-fields' },
            field('serverName', '服务名称（serverName）', '例如 notion'),
            selectField('transport', '传输方式', [['streamable-http', '可流式 HTTP（Streamable HTTP）'], ['stdio', '标准输入输出（stdio，本地命令）']]),
            form.transport === 'streamable-http' && selectField('authType', '认证方式', [['none', '无认证'], ['oauth', 'OAuth 2.1 / PKCE'], ['bearer', 'Bearer 令牌']]),
            form.transport === 'streamable-http' && field('resourceUrl', 'MCP 服务地址', 'https://example.com/mcp', true),
            form.transport === 'streamable-http' && form.authType === 'bearer' && field('bearerToken', 'Bearer 令牌', '令牌内容（无需填写 Bearer 前缀）', true, 'password'),
            form.transport === 'streamable-http' && field('headersText', '自定义请求头（可选，每行“名称: 值”）', 'X-API-Key: xxx', true, 'textarea'),
            form.transport === 'streamable-http' && form.authType === 'oauth' && field('scope', 'OAuth 权限范围（可选）', '例如 tools.read'),
            form.transport === 'streamable-http' && form.authType === 'oauth' && field('clientId', '客户端 ID（可选）', '支持动态注册时可留空'),
            form.transport === 'streamable-http' && form.authType === 'oauth' && field('clientSecret', '客户端密钥（可选）', '', false, 'password'),
            ...
            form.transport === 'stdio' && field('envText', '环境变量（可选，每行“名称=值”）', 'API_KEY=xxx', true, 'textarea'),
